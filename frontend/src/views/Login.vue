<template>
  <div class="auth-wrapper">
    <div class="auth-card">
      ...
      <p class="switch-text">New to system? <router-link to="/register">Create an account</router-link></p>
    </div>
  </div>
</template>
...
const handleLogin = async () => {
  loading.value = true
  try {
    const res = await axios.post('http://localhost:8080/api/v1/auth/login', form.value)
    localStorage.setItem('crm_token', res.data.token)
    localStorage.setItem('crm_username', res.data.username)
    localStorage.setItem('crm_role', res.data.role_id)
    
    alert(res.data.message)
    router.push('/dashboard')
  } catch (err) {
    alert(err.response?.data?.error || 'Login Failed. Check connection.')
  } finally {
    loading.value = false
  }
}
...
<style scoped>
.auth-wrapper { display: flex; justify-content: center; align-items: center; height: 100vh; background: transparent; font-family: 'Inter', sans-serif; }
